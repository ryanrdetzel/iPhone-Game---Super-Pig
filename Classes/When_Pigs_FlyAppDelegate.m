...
@synthesize enemys,playerBullets,grass1,grass2,heart,info;
...
-(IBAction)showMenu:(id)sender{
	...
	if (newLevels > 0){
		...
		[self alert:@"New Levels Unlocked" message:[NSString stringWithFormat:@"%d new level%@ been unlocked for you to explore!",newLevels,ss]];
...
-(IBAction)showWindow1:(id)sender{
	...
	if ([cname isEqualToString:@"usa"])				[countryLabel setText:@"United States"]; 
	else if ([cname isEqualToString:@"england"])	[countryLabel setText:@"England"]; 
	else if ([cname isEqualToString:@"brazil"])		[countryLabel setText:@"Brazil"]; 
	else if ([cname isEqualToString:@"canada"])		[countryLabel setText:@"Canada"]; 
	else if ([cname isEqualToString:@"ethiopia"])	[countryLabel setText:@"Ethiopia"]; 
	else if ([cname isEqualToString:@"greenland"])	[countryLabel setText:@"Greenland"]; 
	else if ([cname isEqualToString:@"india"])		[countryLabel setText:@"India"]; 
	else if ([cname isEqualToString:@"israel"])		[countryLabel setText:@"Israel"]; 
	else if ([cname isEqualToString:@"japan"])		[countryLabel setText:@"Japan"]; 
	else if ([cname isEqualToString:@"norway"])		[countryLabel setText:@"Norway"]; 
	else if ([cname isEqualToString:@"russia"])		[countryLabel setText:@"Russia"]; 
	else if ([cname isEqualToString:@"tanzania"])	[countryLabel setText:@"Tanzania"]; 
	else if ([cname isEqualToString:@"togo"])		[countryLabel setText:@"Togo"]; 
	else if ([cname isEqualToString:@"australia"])	[countryLabel setText:@"Australia"]; 
}
...
-(void)initStuff{
	...
	[info setText:@""];
	[info setFrame:CGRectMake(-35, 270, 100, 30)];
	[info setTextAlignment:UITextAlignmentCenter];
	[info setFont:[UIFont fontWithName:@"Marker Felt" size:18]];
	[info setTextColor:[UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1]];
	[info setBackgroundColor:[UIColor clearColor]];
	[info setAdjustsFontSizeToFitWidth:YES];
	
	points.transform = rotate;
	info.transform = rotate;
...
-(void)loadLevel:(NSString *)name{
	...
	mainView = [[PFObject alloc] init:skyline x:0 y:5 isStatic:YES type:MISC]; 
		...
		for(int x=0;x<[scores count];x++){
			if ([[scores objectAtIndex:x] status] == DEAD){
				...
				if (p < 0){
					...
					[[scores objectAtIndex:x] setVelocity:CGPointMake(40,-70)];
					[[scores objectAtIndex:x] setType:HEALTH];
				}
...
-(void)applyBonus:(int)b obj:(PFObject *)obj{
	if (b >= 50){
		...
		switch(b){
			...
			case BONUS_HEART1: 
				[self addPoints:25 location:CGPointMake([obj center].x,[obj center].y-15)]; 
				[player addHealth:25];
				if ([player health] > 100){
					[player setHealth:100];
				}
				[self addDamage:25 location:CGPointMake(player.center.x + 25,player.center.y-20)];
		...
		if (b == JESTER_HAT){
			...
			doubleDamage = YES;
			bulletBounce = NO;
			[info setText:@"DOUBLE DAMAGE"];
		}
		else if (b == SANTA_HAT){
			[player putHatOn:@"hat-santa"];
			[self addPoints:50 location:CGPointMake([obj center].x,[obj center].y-15)]; 
			time = 15;
			//tripleDamage = YES;
			//bulletBounce = NO;
			invincible = YES;
			[info setText:@"INVINCIBLE"];
		}
		else if (b == CHEF_HAT){
			[player putHatOn:@"hat-chef"];
			[self addPoints:50 location:CGPointMake([obj center].x,[obj center].y-12)]; 
			time = 15;
			tripleDamage = YES;
			bulletBounce = NO;
			[info setText:@"TRIPLE DAMAGE"];
		}
		if (bonusTimer){
...
-(void)addBonus:(NSMutableArray *)list{
	...
	for(int x=0;x<[list count];x++){
		...
		switch(type){
			...
			case SANTA_HAT:		imgName = @"hat-santa.png";		break;
			case CHEF_HAT:	    imgName = @"hat-chef.png";		break;
			...
			case BONUS_HEART1:  imgName = @"heart.png"; break;
		}
...
-(void)addEnemys:(NSMutableArray *)list{
	...
	for(int x=0;x<[list count];x++){
		...
		if (arcadeMode){
			[ship setHealth:ship.health * 2];
			[ship setMaxHealth:ship.maxHealth * 2];
		}
...
-(void)showExplosion:(CGPoint)location{
	...
	if (explosion1 == nil){
		...
		for (int x=0;x<120;x+=5){
			...
			if (x < 10){
				if (x == 0) name = [NSString stringWithFormat:@"explosion0001.png",x];
				else		name = [NSString stringWithFormat:@"explosion000%d.png",x];
			}
			else if (x > 99){
	...
	if (gameRunning){
		...
		if (training){
			if (stage == 1){
				...
				[self pause];
				trainingText.text = @"Great! Super Pig can move left and right too. To move Super Pig to the right simply tilt your iPhone/iPod Touch to the right and to move Super Pig to the left tilt your iPhone/iPod Touch to the left. The more you tilt the faster Super Pig will fly.\n\nTo complete this lesson move Super Pig all the way to the right and then back to the left.";
				[window addSubview:trainingWindow];
			}
			else if (stage == 6){
				[fire setEnabled:NO];
		...
		for(int x=0;x<[enemys count];x++){
			...
			if ([player checkCollisionWithObject:[enemys objectAtIndex:x]]){
				...
				if (!invincible){
					...
					[self addDamage:-25 location:CGPointMake(player.center.x+25,player.center.y+40)];
					[healthBar setValue:[player health]];
					[healthBar setNeedsDisplay];
					noBalls = YES;
		...
		for(int x=0;x<[enemyBullets count];x++){
			...
			if ([player checkCollisionWithObject:[enemyBullets objectAtIndex:x]]){
				if (!invincible){
					[player takeDamage:5];
					[self addDamage:-5 location:CGPointMake(player.center.x+25,player.center.y+40)];
					[healthBar setValue:[player health]];
					[healthBar setNeedsDisplay];
					SoundEngine_Vibrate();
				}
...
-(void)showFinalWindow{
	...
	if ([cname isEqualToString:@"usa"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"togo"]){	
		[self unlockLevel:@"togo"];
		newLevels = 1;
	}
	else if ([cname isEqualToString:@"greenland"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"norway"]){
		[self unlockLevel:@"norway"];
		newLevels = 1;	
	}
	else if ([cname isEqualToString:@"brazil"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"canada"]){
		[self unlockLevel:@"canada"];
		newLevels = 1;
	}
	else if ([cname isEqualToString:@"canada"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"israel"]){
		[self unlockLevel:@"israel"];
		newLevels = 1;
	}
	else if ([cname isEqualToString:@"ethiopia"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"tanzania"]){
		[self unlockLevel:@"tanzania"];
		newLevels = 1;
	}
	else if ([cname isEqualToString:@"israel"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"australia"])	{
			[self unlockLevel:@"australia"];
			newLevels = 1;
	}
	else if ([cname isEqualToString:@"japan"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"ethiopia"]){
			[self unlockLevel:@"ethipoia"];
			newLevels = 1;
	}
	else if ([cname isEqualToString:@"norway"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"japan"])	{
			[self unlockLevel:@"japan"];
			newLevels = 1;
	}
	else if ([cname isEqualToString:@"russia"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"england"])	{
		[self unlockLevel:@"england"];
		newLevels = 1;
	}
	else if ([cname isEqualToString:@"tanzania"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"russia"]){
			[self unlockLevel:@"russia"];
			newLevels = 1;
	}
	else if ([cname isEqualToString:@"togo"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"brazil"]){
			[self unlockLevel:@"brazil"];
			newLevels = 1;
	}
	else if ([cname isEqualToString:@"australia"] && ![[NSUserDefaults standardUserDefaults] stringForKey:@"india"]){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{	
	...
	NSString *myId = [NSString stringWithFormat:@"id",index];
